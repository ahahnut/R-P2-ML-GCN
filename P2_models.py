...
class GCNResnet(nn.Module):
    ...
    def forward(self, feature, inp):
        ...
        feature = torch.add(feature,noise)
        # print(feature.shape)

        inp = inp[0]
        adj = gen_adj(self.A,self.num_classes).detach()
        self.x = self.gc1(inp, adj)
        self.x = self.relu(self.x)
        self.x = self.gc2(self.x, adj)

        self.x = self.x.transpose(0, 1)
        self.x = torch.matmul(feature, self.x)
        #noise = torch.from_numpy(np.random.laplace(0,1/epsilon,self.x.size())).cuda().float()
